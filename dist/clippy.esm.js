...
Balloon.prototype.speak = function speak (complete, text, hold, callback) {
    this._hidden = false;
    this.show();
    var c = this._content;
    // set height to auto
...
Balloon.prototype._sayWords = function _sayWords (text, choices, hold, complete, callback, isQuestion) {
    this._active = true;
    this._hold = hold;
    ...
    this._addWord = $.proxy(function () {
        ...
        if (idx <= words.length) {
            ...
            // No more words left
            choices.length && el.append('<div class="questions" />');
...
var Agent = function Agent (path,data,sounds) {
    ...
    this._balloonQueue = new Queue($.proxy(this._onQueueEmpty, this));
...
Agent.prototype.moveTo = function moveTo (x, y, duration, finished) {
    var dir = this._getDirection(x, y);
    var anim = 'Move' + dir;
    if (duration === undefined) { duration = 1000; }

    this._addToQueue(function (complete) {
        // instant reposition - the simple case
        if (duration === 0) {
    ...
    {
        // this._addToQueue(finished, this);
    }
};
...
Agent.prototype.speak = function speak (text, hold, callback) {
    this._addToBalloonQueue(function (complete) {
 ...
 */
Agent.prototype.closeBalloon = function closeBalloon (fast) {
...
Agent.prototype._addToBalloonQueue = function _addToBalloonQueue (func, scope) {
    if (scope) { func = $.proxy(func, scope); }

    this._balloonQueue.queue(func);
};
